...
* Modularize the setup script such that each application is self-contained and is merely "plugged" into the installation process. Thus give the option to install only parts of the dotfiles (e.g. omit `i3` installation if user does not wand to install this window manager).
